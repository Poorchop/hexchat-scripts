...
unknown_3_regex    = re.compile(".*?For\sMy\sList.*?\([\w:]+\)\sand\sDCC\sStatus,\stype\s[@\w\\\[\]{}^`|-]+\sand\s[@\w\\\[\]{}^`|-]+\.\s.*?Slots.*?Ques\sTaken.*?Next\sSend:.*?CPS\sin\sUse:.*?Highest\sCps\sRecord:.*?Total\sFile\sServed:.*?")
